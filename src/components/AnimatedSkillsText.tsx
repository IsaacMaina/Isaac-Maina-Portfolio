...
interface AnimatedSkillsTextProps {
  skills?: string[];
}

const AnimatedSkillsText = ({ skills = ['Web Developer', 'IT Support', 'Data Analyst'] }: AnimatedSkillsTextProps) => {
  ...
  // Filter out empty skills and ensure we have at least one value
  const validSkills = skills && skills.length > 0 ? skills.filter(skill => skill.trim() !== '') : ['Developer'];
  ...
  useEffect(() => {
    ...
    if (isDeleting) {
      ...
      if (currentText.length > 0) {
        ...
        setIsDeleting(false);
        setCurrentIndex((prev) => (prev + 1) % validSkills.length);
  ...
  }, [currentText, isDeleting, currentIndex, typingSpeed, validSkills, isMobile, reducedMotion]);
...
export default AnimatedSkillsText;